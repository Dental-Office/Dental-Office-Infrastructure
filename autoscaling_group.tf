resource "aws_autoscaling_group" "backend_app_autoscaling" {
  min_size             = 2
  max_size             = 4
  desired_capacity     = 2
  launch_template {
    id = aws_launch_template.launchtemplate_back13.id 
  }
...
resource "aws_autoscaling_policy" "cpu-policyscaleup" {
  name = "cpu-policy"
  autoscaling_group_name = aws_autoscaling_group.backend_app_autoscaling.id
...
resource "aws_cloudwatch_metric_alarm" "cpu-alarm-scaleup" {
  ...
  threshold = 40
  dimensions = {
  "AutoScalingGroupName" = "${aws_autoscaling_group.backend_app_autoscaling.name}"
  }
...
resource "aws_autoscaling_policy" "cpu-policy-scaledown" {
  ...
  autoscaling_group_name = "${aws_autoscaling_group.backend_app_autoscaling.name}"
  adjustment_type = "ChangeInCapacity"
  scaling_adjustment = -1
  cooldown = 300
  policy_type = "SimpleScaling"
